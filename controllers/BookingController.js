    ...
    await appointment
      ...
      .catch((err) => {
        res.status(500).send({
          message:
            err.message ||
            "Some error occurred while saving the appointment info into database",
        });
      });
  }
};
...
const approveBooking = async (req, res) => {
  ...
  const appointment = await Appointment.findByIdAndUpdate(appointment_id, {
    ...
    .populate("patient", { email: 1 })
    .then((data) => {
      sgMail.setApiKey(process.env.SENDGRID_API_KEY);
      const msg = {
        to: data.patient.email,
        from: "aaraf.islam2@gmail.com", // Use the email address or domain you verified above
...
const adminviewapprovedappointments = async (req, res) => {
  ...
  if (!approvedappointments) {
    ...
    console.log("approvedappointments", approvedappointments);
    res.render("approveBookingsView", {
      title: "MediPal UTM",
      approvedappointments: approvedappointments,
    });
  }
  ...
  if (approvedappointment.status == "called") {
    ...
    await Appointment.findByIdAndUpdate(appointment_id, {
      ...
      .then((appointment) => {
        ...
        eventEmitter.emit(`booking.call.${appointment.patient._id}`, {
          message: `Doctor is calling from Room no. ${req.session.user.room_no}`,
        });
...
const discardBooking = async (req, res) => {
  ...
  const appointment = await Appointment.findByIdAndUpdate(appointment_id, {
    ...
    .then(async (data) => {
      ...
      const msg = {
        to: data.patient.email,
        from: "aaraf.islam2@gmail.com.com", // Use the email address or domain you verified above
...
module.exports = {
  ...
  approveBooking,
  rejectBooking,
  findApprovedAppointments,
  callBooking,
  adminviewapprovedappointments,
  discardBooking,
};
