...
const addactivity = (req, res, next) => {
  if (!req.body) {
    req.flash(
      "message",
      "Field can not be empty! Please provide file before submit"
    );
...
const createDoctor = (req, res) => {
  ...
  req
    .check("username", "Field can not be empty, It is required")
    .not()
    .isEmpty();
  ...
  req
    .check("password", "Field can not be empty, It is required")
    .not()
    .isEmpty();
  ...
  req
    .check("phone_no", "Field can not be empty, It is required")
    .not()
    .isEmpty();
  req
    .check("sitting_time", "Field can not be empty, It is required")
    .not()
    .isEmpty();
  req
    .check("room_no", "Field can not be empty, It is required")
    .not()
    .isEmpty();

  req.check("email", "Invalid Email").isEmail();
  req
    .check("password", "Password should not be longer than 8 characters")
    .isLength({ max: 8 });
    ...
    doctor
      ...
      .catch((err) => {
        res.status(500).send({
          message:
            err.message ||
            "Some error occurred while saving the doctor info into database",
        });
      });
  }
};
...
const findDoctor = (req, res) => {
  Doctor.find({})
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Error Occurred while retriving doctor info from DB",
      });
    });
};
...
const updateDoctor = (req, res) => {
  if (!req.body) {
    return res
      .status(400)
      .send({ message: "Data/field to be updated can not be empty" });
  }
  ...
  Doctor.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then((doctor) => {
      if (!doctor) {
        res.status(404).send({
          message: `Can not Update the doctor with ${id}. Maybe user not found!`,
        });
      } else {
        res.send(doctor);
      }
    ...
    .catch((err) => {
      res
        .status(500)
        .send({ message: err.message || "Error! can not update info" });
    });
};
    ...
    pharmacist
      ...
      .catch((err) => {
        res.status(500).send({
          message:
            err.message ||
            "Some error occurred while saving the pharmacist info into database",
        });
      });
  }
};
  ...
  Pharmacist.find({})
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Error Occurred while retriving pharmacist info from DB",
      });
    });
};
...
const updatePharmacist = (req, res) => {
  ...
  Pharmacist.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then((pharmacist) => {
      if (!pharmacist) {
        res.status(404).send({
          message: `Can not Update the pharmacist with ${id}. Maybe user not found!`,
        });
      } else {
        res.send(pharmacist);
      }
