...
const createPrescription = async (req, res) => {
  ...
  if (!prescription) {
    req.flash(
      "message",
      "Please provide something in the field! Fields can not be empty"
    );
...
const viewprescription = async (req, res) => {
  ...
  if (prescription.status == "viewed") {
    res
      .status(400)
      .send({ message: "This prescription has been viewed already" });
    return;
  } else {
...
const findPrescriptionsWithMatric = async (req, res) => {
  ...
  if (patient) {
    const prescription = await Prescription.find({
      ...
      .limit(1)
      .lean()
      .populate("patient"); //gets the latest one
    if (!prescription) {
      ...
      res.render("doctorPrescriptionPage", {
        title: "MediPal UTM",
...
const findpatientprescriptions = async (req, res) => {
  ...
  if (patient_id) {
    const prescription = await Prescription.find({
      ...
      .then((prescription) => {
        console.log("prescription", prescription);
        res.render("viewDiagnosticReport", {
          title: "MediPal UTM",
          layout: "dashboardlayout",
          prescription: prescription,
        });
...
const viewpatientprescription = async (req, res) => {
  ...
  const prescription = await Prescription.findById(prescription_id)
    ...
    .then((prescription) => {
      console.log("prescription", prescription);
      res.render("patientPrescriptionPage", {
        title: "MediPal UTM",
        prescription: prescription,
      });
    })
